...
import { approveAndMapTokenHashToTokens, setStrategy } from "../helpers/contracts-actions";
...
describe(scenario.title, async () => {
  ...
  before(async () => {
    try {
      ...
      const riskManager = await deployRiskManager(hre, owner, TESTING_DEPLOYMENT_ONCE, registry.address, 1);
